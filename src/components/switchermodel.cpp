...
{
    qDebug() << Q_FUNC_INFO << "Updating window list" << endl;
    ...
    SWITCHER_DEBUG("Read list of " << numWindowItems << " windows" << endl);
    ...
    for (unsigned int i = 0; i < numWindowItems; i++) {
        ...
        if (wi->visibleInSwitcher()) {
            ...
            if(wi->pid()>0) {
                ...
                Application* app = ApplicationManager::instance()->findApplication(wi->commandLine());
                if (app != NULL) {
                     wi->setDesktopEntry(app->getDesktopEntry());
                }
