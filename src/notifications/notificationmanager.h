...
class  NotificationManagerAdaptor;
...
public:
    ...
    /*!
     * Sends a missed call notification to the notification server.
     *
     * \param from Telephone number or contact name.
     */
    void notifyCall(const QString &from);

    /*!
     * Sends a SMS notification to the notification server.
     *
     * \param from Telephone number or contact name.
     */
    void notifySMS(const QString &from);


    /*!
     * Remove all the missed call notifications.
     */
    void clearCalls();

    /*!
     * Remove all the received SMS notifications.
     */
    void clearSMSs();
    ...
    /*!
     * Returns all the missed call notifications.
     */
    QList<Call*> getCalls();

    /*!
     * Returns all the received SMS notifications.
     */
    QList<SMS*> getSMSs();


    /*!
     * Reload feed templates
     */
    void reloadFeedTemplates();
...
signals:
    ...
    /*!
      * Emitted when calls notification list is modified (item added or removed)
      */
    void callsModified();

    /*!
      * Emitted when SMSs notification list is modified (item added or removed)
      */
    void smsModified();

    /*!
      * Emitted when feed templates are  modified
      */
    void feedTemplatesModified();
...
public slots:
    ...
    void serviceOwnerChange(QString service, QString oldOwner, QString newOwner);
...
private:
    ...
    //! List of all missed calls
    QList<Call*> calls;

    //! List of all received SMS which have not been read
    QList<SMS*> SMSs;
    ...
    NotificationManagerAdaptor* adaptor;
